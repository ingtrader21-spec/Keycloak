...
class AggregateTests(unittest.TestCase):
    def test_only_applicable_successes_pass(self):
        ...
        for event in ['pull_request', 'push', 'workflow_dispatch']:
            for source, merge, contract in itertools.product(states, repeat=3):
                expected = source == contract == 'success' and (
                    event == 'pull_request' and merge == 'success' or
                    event == 'push' and merge == 'skipped')
                with self.subTest(event=event, source=source, merge=merge, contract=contract):
                    result = subprocess.run(['bash', '-c', script], env={
                        **os.environ, 'EVENT_NAME': event, 'SOURCE_RESULT': source,
                        'MERGE_RESULT': merge, 'CONTRACT_RESULT': contract,
                    }, capture_output=True)
