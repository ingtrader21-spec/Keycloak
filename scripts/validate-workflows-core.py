...
def validate_source_workflow(path: Path, workflow: dict[str, Any]) -> None:
    ...
    if set(jobs) != {"validate-source", "validate-merge-result", "validate"}:
        fail(f"{path}: expected source, merge-result and aggregate validate jobs")
    ...
    if aggregate.get("if") != "always()" or set(aggregate.get("needs", [])) != {
        "validate-source", "validate-merge-result"
    }:
