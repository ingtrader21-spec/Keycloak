...
[[ "$(jq -er '.driftCount' "$plan_dir/plan.json")" -eq 25 ]]
...
[[ "$(jq -er '.absentCreatableClientCount' "$rollback_dir/rollback-metadata.json")" -eq 24 ]]
...
for client_id in grafana superset openbao; do
  jq -e --arg client_id "$client_id" '
    ...
    and .[$client_id].representation.publicClient == false
    and .[$client_id].representation.directAccessGrantsEnabled == false
    and .[$client_id].representation.serviceAccountsEnabled == false
    and .[$client_id].representation.attributes["pkce.code.challenge.method"] == "S256"
    and .[$client_id].representation.attributes["access.token.lifespan"] == "300"
    and (.[$client_id].representation | has("secret") | not)
  ' "$state_file" >/dev/null
done
